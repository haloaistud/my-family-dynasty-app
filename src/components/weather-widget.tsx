...
export default function WeatherWidget() {
    ...
    useEffect(() => {
        ...
        function createRaindropsForCloud(cloud: THREE.Group) {
            ...
            cloud.add(rainGroup);
            (cloud.userData.rainGroup as THREE.Group) = rainGroup;
            const raindropMaterial = new THREE.MeshBasicMaterial({
                color: cloud.userData.rainColor,
                transparent: true,
                opacity: 0.7
            });
        ...
        const onClick = (event: MouseEvent) => {
            ...
            if (intersects.length > 0) {
                let clickedObj: THREE.Object3D = intersects[0].object;
        ...
        <div className="relative rounded-lg border bg-card text-card-foreground shadow-sm p-6 weather-widget overflow-hidden h-64">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
                <div className="flex justify-between items-start">
                    <div>
                        <h2 className="text-2xl font-bold">Cloudy</h2>
                        <p id="dateTime" className="text-sm text-muted-foreground">{dateTime}</p>
                    </div>
                    <div className="text-4xl font-bold">19°</div>
                </div>
                <div id="cloud-tooltip" className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/50 text-white text-xs rounded-md px-2 py-1 opacity-0 transition-opacity duration-300 pointer-events-none">
